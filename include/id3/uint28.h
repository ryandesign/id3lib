...
#ifndef ID3LIB_UINT28_H
#define ID3LIB_UINT28_H
...
namespace id3
{
  ...
  public:
    ...
    uint28(const bstring& data) { *this = data; }
    uint28(const uint28& rhs) : __value(rhs.to_uint32()) { ; }
    ~uint28() { ; }
    
    uint28&    operator=(const bstring&);
    ...
    size_t    parse(const bstring& data) 
    { 
      *this = data; 
      return std::min(data.size(), sizeof(uint32)); 
    }
